			...
			if (!oldScheList.contains(newScheList.get(i)) && newScheList.get(i).getScheduleID() == 0) {
				...
				if (flag) {
					...
					if (oldScheList.isEmpty()) {
						...
						LocalDate minStart = oldScheList.stream().map(Schedule::getStartDate).min(LocalDate::compareTo)
								.get();
						LocalDate maxEnd = oldScheList.stream().map(Schedule::getEndDate).max(LocalDate::compareTo)
								.get();
						if (!newScheList.get(i).getEndDate().isAfter(minStart)
							...
							if (newScheList.get(i).getNumberStaff() > staffTotal) {
								...
								errors.rejectValue("schedules[" + i + "].numberStaff", null, "Staff productivity is not fully defined. Please check.");
							}else{
